...
set PROJECT_PATH=O:\Git\MauiApp3\WebApplication1\
...
:: update version
powershell -Command "(Get-Content '%CSPROJ_FILE%') -replace '<AssemblyVersion>.*</AssemblyVersion>', '<AssemblyVersion>%NEW_VERSION%</AssemblyVersion>' -replace '<FileVersion>.*</FileVersion>', '<FileVersion>%NEW_VERSION%</FileVersion>' | Set-Content '%CSPROJ_FILE%'"
